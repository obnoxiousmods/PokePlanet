...
void MmoPlayers_Update(void)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        // Keep this player's floating name tag on their sprite. The tag is a small 32x16 sprite
        // now (eight OBJ tiles, not thirty-two) and follows in camera space, so it neither starves
        // the remote-player sprites of tiles nor drifts off to a corner -- the two faults that had
        // it disabled. ApplyRemote may not have a sprite yet (no free object-event slot this
        // frame), in which case there is nothing to tag.
        {
            struct ObjectEvent *object = sSlots[slot].spawned ? FindSlotObject(slot) : NULL;
